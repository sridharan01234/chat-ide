...
import { DropZoneItem, ChatItem, ChatMessage, DRAG_DROP_MIME_TYPES } from './types';
 ...
 */
export class DropZoneProvider implements vscode.TreeDataProvider<DropZoneItem>, vscode.TreeDragAndDropController<DropZoneItem> {
    ...
    public async handleDrop(target: DropZoneItem | undefined, dataTransfer: vscode.DataTransfer, token: vscode.CancellationToken): Promise<void> {
        ...
        try {
            ...
            vscode.window.showErrorMessage(`Failed to process dropped files: ${ErrorUtils.createUserFriendlyError(error)}`);
        }
    }
 ...
 */
export class FileTreeDataProvider implements vscode.TreeDataProvider<vscode.Uri> {
    ...
    getAttachedFiles(): vscode.Uri[] {
        return Array.from(this.attachedFiles).map(uriString => vscode.Uri.parse(uriString));
    }
