...
export class FileManager {
  ...
  async createFileReference(
    ...
    try {
      ...
      throw new Error(
        `Failed to create file reference for ${filePath}: ${ErrorUtils.createUserFriendlyError(error)}`,
      );
    }
  }
   ...
   */
  createFileReferenceFromDocument(
  ...
  async readFileFromUri(uri: vscode.Uri): Promise<string> {
    try {
      ...
      throw new Error(
        `Failed to read file ${uri.fsPath}: ${ErrorUtils.createUserFriendlyError(error)}`,
      );
    }
  }
  ...
  private async readFileContent(filePath: string): Promise<string> {
    try {
      ...
      throw new Error(
        `Failed to read file ${filePath}: ${ErrorUtils.createUserFriendlyError(error)}`,
      );
    }
  }
  ...
  private validateFileContent(content: string, fileName: string): void {
    ...
    if (!ValidationUtils.isValidFileSize(fileSize, this.maxFileSize)) {
      throw new Error(
        `File ${fileName} is too large (${FileUtils.formatFileSize(fileSize)}). Maximum allowed size is ${FileUtils.formatFileSize(this.maxFileSize)}.`,
      );
    }
    ...
    if (ValidationUtils.isBinaryContent(content)) {
      throw new Error(
        `File ${fileName} appears to be a binary file and cannot be processed.`,
      );
    }
  }
  ...
  async processDroppedUri(uri: vscode.Uri): Promise<FileReference[]> {
    try {
      ...
      throw new Error(
        `Failed to process ${uri.fsPath}: ${ErrorUtils.createUserFriendlyError(error)}`,
      );
    }
  }
  ...
  private async processDirectory(uri: vscode.Uri): Promise<FileReference[]> {
    try {
        ...
        if (fileType === vscode.FileType.File) {
          ...
          if (FileUtils.isSupportedFileExtension(fileExtension)) {
            try {
              ...
              console.warn(
                `Skipping file ${fileName}: ${ErrorUtils.createUserFriendlyError(error)}`,
              );
            }
          }
        }
      ...
      if (processedFiles.length === 0) {
        throw new Error(
          `No supported files found in folder ${path.basename(uri.fsPath)}`,
        );
      }
      ...
      ErrorUtils.logError("FileManager.processDirectory", error);
      throw new Error(
        `Failed to process directory ${uri.fsPath}: ${ErrorUtils.createUserFriendlyError(error)}`,
      );
    }
  }
  ...
  async getFileInfo(
    uri: vscode.Uri,
  ): Promise<{ name: string; size: number; type: vscode.FileType }> {
    try {
      const stat = await vscode.workspace.fs.stat(uri);
      return {
      ...
      throw new Error(
        `Failed to get file info for ${uri.fsPath}: ${ErrorUtils.createUserFriendlyError(error)}`,
      );
    }
  }
