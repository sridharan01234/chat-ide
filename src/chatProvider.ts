...
export class ChatProvider implements vscode.WebviewViewProvider {
  ...
  private async handleSendMessage(text: string): Promise<void> {
    ...
    try {
      ...
      const fileReference =
        attachedFiles.length > 0 ? attachedFiles[0] : undefined;
...
`;
  ...
  private async handleAttachFile(): Promise<void> {
    ...
    try {
      const fileReference = this.fileManager.createFileReferenceFromDocument(
    ...
    try {
      ...
      for (const filePath of files) {
        try {
          ...
          console.error(
            "[ChatProvider] Error processing file:",
            filePath,
            error,
          );
      ...
      if (fileReferences.length > 0) {
        console.log(
          "[ChatProvider] Successfully processed files:",
          fileReferences.map((f) => f.fileName),
        );
      ...
      vscode.window.showErrorMessage(
        `Failed to attach files: ${ErrorUtils.createUserFriendlyError(error)}`,
      );
    }
  }
